...
$(document).ready(function(){
	...
	function sumarPuntos(){
		...
		var randNum = Math.round(Math.random()*250);
		var randNum2 = Math.round(Math.random()*300);
		...
		if (puntos == necesarios){
			...
			necesarios+= 10;
			level += 1;
			clicks += necesarios - 10;
			document.getElementById("level").innerHTML = level;
			document.getElementById("masdiez").innerHTML = '+ '+mastiempo+'s';
			document.getElementById("level1").innerHTML = level;
			win.addClass('ganador');
		}else{
			win.removeClass('ganador');
		}
	}
	function restarTiempo(){
		tiempo--;
		document.getElementById("tiempo").innerHTML = '&nbsp;&nbsp; <i class="far fa-clock"></i> : ' + tiempo +'s';
		if (tiempo <= 10){
			$('#tiempo').addClass('tiempotermina');
		}else{
			$('#tiempo').removeClass('tiempotermina');
		}
		if (tiempo == 0){
			tiempo = 60;
			mastiempo = 10;
			...
			clearInterval(a);
			let modal = `
				<div class="louser" id="modal">
					<div class="cont-louser">
						<h2>Louser</h2>
						<div class="info-score">
							...
							<div class="redes-game">
								<a href="https://www.instagram.com/jahen.multimedia/" class="insta" target="_blank"><i class="fab fa-instagram"></i></a>
								<a href="https://www.facebook.com/JAHEN-105275760909711/?__tn__=%2Cd%2CP-R&eid=ARBGY83wyjlTrox29DCTViQHvDvz-mVBAOFf2_2TWk7Fu6PE5rL5h6IjD-FRds4SKPLWpKMafHQ5FweW" class="face" target="_blank"><i class="fab fa-facebook-square"></i></a>
								<a href="https://www.youtube.com/channel/UCwaAnir4C8eDI2JAbVnXpWg" class="you" target="_blank"><i class="fab fa-youtube"></i></a>
							</div>
							<div class="botones-louser">
								<button id="jugarOtra">Jugar</button>
	...
	$('#playGame').on('click', function(){
		$('.cont-juego').css('display', 'flex');
		disminuirTiempo();
	});
	$('.close-juego').on('click', function(){
		$('.cont-juego').css('display', 'none');
		clearInterval(a);
		tiempo = 60;
		puntos = 0;
